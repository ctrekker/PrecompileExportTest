...
using Images, FileIO, Base64, HypertextLiteral, PlutoUI
...
rule_number = (_rule_number isa Int) ? _rule_number : 0

# ╔═╡ 4cfb2077-4bec-4c36-b6fa-4644a5ba98bd
function totalistic_rules(code::Int, colors::Int)
...
# ╔═╡ ca21379b-2636-44b5-ab41-5f510f24b6c9
rules = totalistic_rules(rule_number, colors)
# rules = [0x0, 0x0, 0x2, 0x3, 0x1, 0x2, 0x0, 0x0, 0x0, 0x0]

# ╔═╡ 7d0cb68c-472f-4683-a7f1-227c63160987
function simulate(steps::Int, rules, colors; initial_color=1)
	state = zeros(UInt8, steps, steps * 2 - 1)
	initial_state = zeros(UInt8, steps * 2 - 1)
	initial_state[steps] = initial_color
	state[1, :] .= initial_state

	side_state = 0
	
	for step ∈ 2:steps
		next_state = [rules[state[step-1, n] + (n > 1 ? state[step-1, n-1] : side_state) + (n < size(state, 2) ? state[step-1, n+1] : side_state) + 1] for n ∈ 1:size(state, 2)]
		side_state = rules[3 * side_state + 1]
...
# ╔═╡ a074bfeb-e1b1-4e82-983b-9b2b6a7029ff
function pattern_filter(state::Array{T, 2}) where {T}
	[
		(j > 1 && state[i, j-1] == state[i, j]) && (j < size(state, 2) && state[i, j+1] == state[i, j]) ? 0 : state[i, j]
	for i ∈ 1:size(state, 1), j ∈ 1:size(state, 2)]
end

# ╔═╡ c0807722-8df9-4a98-a03d-4e7590e6b610
function render(state::Array{T, 2}, colors) where {T}
	Gray.((state ./ (colors - 1)))
end

# ╔═╡ df8de748-0975-4076-82f5-b5cb87176fae
sim = simulate(500, rules, colors; initial_color=1)

# ╔═╡ 1cc8dc91-0428-4e7a-9e03-45d7ef7556a8
md"""
## Icon Image
"""

# ╔═╡ 40c755dd-38d7-4b23-9279-16495be7b13b
img = render(sim, 4)
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "01cef5c505fb38f9054e6a11a86bb6c337ee3866"
